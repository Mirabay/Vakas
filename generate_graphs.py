...
def plot_bed_subdivisions(df):
    ...
    sns.set_theme(style="whitegrid", palette="pastel")
    bed_counts.plot(
        kind="barh", stacked=True, figsize=(10, 8), color=sns.color_palette("pastel", n_colors=3)
    )
...
def plot_bed_usage_over_time(df, column_name):
    ...
    plt.yticks(ticks=list(reverse_category_mapping.keys()), labels=list(reverse_category_mapping.values()))
